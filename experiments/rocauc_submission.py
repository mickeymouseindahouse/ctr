...
if __name__ == '__main__':
    data_loader = TrainLoaderSessionSplitter(result_path=RESULT_PATH,
                                             ...
                                             preprocessing=BaseModelPipeline(steps=[UnderSampler(), FillNaPreprocessor()]))
    data_loader.load_data()
    X_train, X_test, y_train, y_test = data_loader.split_data()
    data_loader.dump_to_pickle()

    pipeline = BaseModelPipeline(result_path=RESULT_PATH, scoring=roc_auc_score, steps=[
    ...
    pipeline.dump_to_pickle(file_name="rocauc.pkl")
